...
export default function Home() {
  ...
  return (
    <>
      {/* Hero Section */}
      <main
        className="min-h-screen flex flex-col items-center justify-start relative overflow-hidden px-4 md:px-6 lg:px-12"
        style={{
      ...
      >
        ...
        {leftDoodles.map((doodle, idx) => (
          <motion.div
            ...
            animate="float"
            className="absolute w-16 h-16 md:w-20 md:h-20 lg:w-32 lg:h-32 z-20"
            style={{
              top: doodle.top,
              left: doodle.left,
        ...
        {/* Hero Content */}
        <div className="text-center z-30 max-w-4xl pt-8 md:pt-10 lg:pt-14">
          {/* Animated GDG */}
          <div className="relative w-64 h-40 md:w-80 md:h-48 mx-auto mb-4">
            {letters.map((l, i) => (
              <motion.div
                key={i}
                custom={i}
                animate={controls}
                className="w-16 h-24 md:w-20 md:h-28 rounded-2xl flex justify-center items-center text-white text-4xl md:text-5xl font-bold shadow-2xl absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
          ...
          {/* Tagline */}
          <h1 className="text-3xl md:text-5xl lg:text-6xl font-extrabold leading-tight tracking-tight mb-4 text-white drop-shadow-lg px-4">
            <span>OnCampus VIIT</span>
          </h1>
          <p className="text-base md:text-lg lg:text-xl text-gray-200 leading-relaxed drop-shadow-lg px-4">
          ...
          <motion.a
            ...
            className="mt-6 md:mt-8 inline-block bg-gradient-to-r from-[#4285F4] to-[#0F9D58] text-white font-semibold py-2.5 px-5 md:py-3 md:px-6 rounded-full shadow-lg transition-all text-sm md:text-base"
          >
            Explore Events
          </motion.a>
        </div>
